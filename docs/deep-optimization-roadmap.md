...
- 给简历片段、RAG 上下文、AI 顾问工具返回内容设置统一字符或 token 预算。
- RAG 检索结果按来源、相似度、片段长度裁剪，避免把无关长文本塞进 Prompt。
- AI 顾问多轮对话限制历史轮数，必要时保留最近对话 + 当前简历画像摘要。
- 在运营看板展示平均上下文长度，辅助判断 Prompt 是否过长。
...
- 当顾问对话超过固定轮数后，使用模型生成一份会话摘要。
- 后续 Prompt 只携带“会话摘要 + 最近几轮消息 + 当前问题”。
- 摘要内容限定为用户目标、已确认事实、关键建议和未完成事项。
- 摘要结果持久化到会话记录，便于刷新页面后继续对话。
...
- 前端选择一道面试题后录音。
- 后端上传音频，优先使用 DashScope / Spring AI Alibaba 官方语音识别能力转写文本。
- 复用现有 `AnswerEvaluationAgent` 对转写文本评分。
- 增加语音表达建议：语速、停顿、重复词、表达完整度。
- 审计语音转写耗时、失败原因和转写文本长度。
...
- 上传 JD 截图，优先使用 DashScope / Spring AI Alibaba 官方 OCR 或视觉模型能力提取文本。
- OCR 结果进入岗位说明输入框，用户确认后再触发岗位匹配。
- 上传简历截图时，先 OCR 提取文本，再复用现有简历分析流程。
- 对 OCR 结果做最小清洗：去掉按钮、时间、电量、招聘软件界面噪音。
- 页面展示 OCR 置信提示，让用户确认后再提交。
