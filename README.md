...
- AI 顾问问答：基于只读工具查询简历画像、面试题和相似简历片段，支持 Query Rewrite、SSE 流式输出和长对话摘要压缩。
...
岗位定制出题不会直接拿完整 Prompt 做向量检索，而是把检索 query 固定为目标 JD 或岗位核心要求，避免输出格式、题目数量和 JSON 约束污染召回。AI 顾问对话使用 Spring AI 官方 `RewriteQueryTransformer` 做轻量 Query Rewrite，改写后的检索意图会写入顾问消息审计，并在运营看板展示。
