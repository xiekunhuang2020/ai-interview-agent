...
把“偶发结构化输出失败”从单次报错变成可诊断、可回放、可修复的工程闭环。

**改造内容**

- 在审计表或独立表中记录结构化输出失败类型、原始模型输出摘要和 Prompt 版本。
- 将结构化失败细分为：
...
- 对可修复错误增加一次“JSON 修复 Prompt”或 Spring AI 官方结构化输出重试策略。
- 对不可修复错误返回更明确的前端提示，并保留 traceId 方便排查。
- 将失败样例纳入第 12 条 Evaluation Harness，作为 Prompt 回归用例。

**验收标准**

- 前端不再只看到“无法转换为目标 DTO”，而能看到可理解的失败类型。
- Prompt 看板能统计结构化输出失败类型和发生次数。
- 至少沉淀 5 条失败样例，用于后续 Prompt 回归。
- 可修复格式错误支持自动修复一次，修复失败后再返回明确错误。

**面试可讲**

大模型结构化输出不是 100% 稳定的。我没有把失败简单吞掉，而是把结构化失败分类、记录样例，并计划接入一次自动修复和评测回归，让 Prompt 优化有真实失败样例支撑。
...
| 有结构化失败修复闭环 | 可写“结构化输出失败分类、样例回放和自动修复” |
