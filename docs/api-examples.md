...
响应会按 `operationName + promptVersion` 聚合模型调用总数、模型名称、成功率、失败数、平均耗时、输入 Token、输出 Token、总 Token、平均 Token、平均 Prompt 字符数、上下文裁剪次数、输入预算超出次数和超预算未覆盖次数。上下文预算使用 Spring AI 官方 `TokenCountEstimator` 估算 Token，上下文裁剪状态和预算状态也会在最近模型调用列表中展示。
